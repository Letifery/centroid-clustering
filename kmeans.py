...
class KMeans():
	def kmeans(
		...
		ver 		: Specifies the kmeans variant. Currently available are: "llyod", "queen"
		...
		match ver:
			...
			case "queen":
				for z in range(epochs):
					for i in range(len(data)):
						tmp = self.calc_distance(data[i], centroids)
						label[i] = min(range(len((tmp))), key=tmp.__getitem__)
						pos = [data[x] for x in range(len(label)) if label[x] == label[i]]
